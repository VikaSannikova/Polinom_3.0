...
//void Polinom::Simplify() //убрать????????????????????77
//{
...
Polinom& Polinom::operator=(const Polinom &right)
{
	if (this == &right)
		return *this;
	delete polinom;								//вызывается деструктор листа
	polinom = new List<monom>;
...
Polinom& operator+(const Polinom&left, const Polinom&right) //сделала без симплифая
{
	...
	while (current_left != NULL && current_right != NULL)
	{
		...
		{
			res->AddElementToTail(current_left->GetVar() + current_right->GetVar());
...
Polinom& operator-(const Polinom&left, const Polinom&right) //здесь не надо, тк тут оператор +
{
	Polinom *res = new Polinom;
...
{
	Polinom *res = new Polinom; //выделили память
	Node<monom>*current = left.polinom->GetHead();
	while (current != NULL)
	{
...
Polinom& operator+=(Polinom&left, const Polinom&right) //тут симплифай
{
	Node<monom>* current = right.polinom->GetHead();
